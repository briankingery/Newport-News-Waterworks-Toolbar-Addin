...
def Email(tool):
    try:
        ...
        body  = '\n'+tool+'\r\n\n\n\n\n'
        body += 'Release Number:\t2\r\n'
        body += 'Release Date:\t\tMay 13, 2016\r\n\n'
    ...
    def onFocus(self, focused):
        def SimpleAutoFill(layer,field):
            try:
                ...
                # Handles field types not text/string
                self.items = []
                self.item_parts = {}
                featureClass = dict[layer]
                with arcpy.da.SearchCursor(featureClass, field) as cursor:
        ...
        try:
            ...
            print 'Select a Layer and Field before selecting/entering a Keyword.'
        ...
        try:
            try:
                if CHOICE_layer == "Street" and CHOICE_field == "NAME":
                    ...
                    prefixSuffixList = ('N','S','E','W','0','U')
                    entry  = string.split(CHOICE_keyword.upper())

                    # Has prefix and suffix
                    if entry[0] in prefixSuffixList and entry[-1] in prefixSuffixList:
                        prefix = entry[0]
                        name   = ' '.join(entry[1:-2])
                        _type  = entry[-2]
                        suffix = entry[-1]
                        ...
                        print 'Type:  ',_type
                        print 'Suffix:',suffix
                                      ##[PREFIX] = '0' AND [NAME] = 'BAILEY' AND [TYPE] = 'CT' AND [SUFFIX] = '0'
                        where_clause = "[PREFIX] = '" + prefix + "' AND [NAME] = '" + name + "' AND [TYPE] = '" + _type + "' AND [SUFFIX] = '" + suffix + "'"
                    # Has prefix no suffix
                    elif entry[0] in prefixSuffixList and entry[-1] not in prefixSuffixList:
                        prefix = entry[0]
                        name   = ' '.join(entry[1:-1])
                        _type  = entry[-1]
                        print 'Prefix:',prefix
                        ...
                        print 'Type:  ',_type
                        print 'Suffix: N/A'
                                      ##[PREFIX] = '0' AND [NAME] = 'ARMSTEAD' AND [TYPE] = 'LOOP'
                        where_clause = "[PREFIX] = '" + prefix + "' AND [NAME] = '" + name + "' AND [TYPE] = '" + _type + "'"
                    # No prefix has suffix
                    elif entry[0] not in prefixSuffixList and entry[-1] in prefixSuffixList:
                        name   = ' '.join(entry[:-2])
                        _type  = entry[-2]
                        suffix = entry[-1]
                        print 'Prefix: N/A'
                        ...
                        print 'Type:  ',_type
                        print 'Suffix:',suffix
                                      ##[NAME] = 'ROGER PEED' AND [TYPE] = 'DR' AND [SUFFIX] = 'N'                    
                        where_clause = "[NAME] = '" + name + "' AND [TYPE] = '" + _type + "' AND [SUFFIX] = '" + suffix + "'"
                    # No prefix no suffix
                    elif entry[0] not in prefixSuffixList and entry[-1] not in prefixSuffixList:
                        name   = ' '.join(entry[:-1])
                        _type  = entry[-1]
                        print 'Prefix: N/A'
                        print 'Name:  ',name
                        print 'Type:  ',_type
                        print 'Suffix: N/A'
                                      ##[NAME] = 'ROCKY' AND [TYPE] = 'ROAD'                    
                        where_clause = "[NAME] = '" + name + "' AND [TYPE] = '" + _type + "'"
                    ...
                    try:
                        ...
                        # Apply search query for integer
...
class ButtonClass_3_FindPoint(object):
    ...
    def onClick(self):
        try:
            ...
            streetOne = street1.upper()
            streetTwo = street2.upper()

            entry1  = string.split(streetOne)
            # Has prefix and suffix
            if entry1[0] in prefixSuffixList and entry1[-1] in prefixSuffixList:
                prefix = entry1[0]
                name   = ' '.join(entry1[1:-2])
                _type  = entry1[-2]
                suffix = entry1[-1]
                               ##[PREFIX] = '0' AND [NAME] = 'BAILEY' AND [TYPE] = 'CT' AND [SUFFIX] = '0'
                where_clause1 = "[PREFIX] = '" + prefix + "' AND [NAME] = '" + name + "' AND [TYPE] = '" + _type + "' AND [SUFFIX] = '" + suffix + "'"
            # Has prefix no suffix
            elif entry1[0] in prefixSuffixList and entry1[-1] not in prefixSuffixList:
                prefix = entry1[0]
                name   = ' '.join(entry1[1:-1])
                _type  = entry1[-1]
                               ##[PREFIX] = '0' AND [NAME] = 'ARMSTEAD' AND [TYPE] = 'LOOP'
                where_clause1 = "[PREFIX] = '" + prefix + "' AND [NAME] = '" + name + "' AND [TYPE] = '" + _type + "'"
            # No prefix has suffix
            elif entry1[0] not in prefixSuffixList and entry1[-1] in prefixSuffixList:
                name   = ' '.join(entry1[:-2])
                _type  = entry1[-2]
                suffix = entry1[-1]
                               ##[NAME] = 'ROGER PEED' AND [TYPE] = 'DR' AND [SUFFIX] = 'N'                    
                where_clause1 = "[NAME] = '" + name + "' AND [TYPE] = '" + _type + "' AND [SUFFIX] = '" + suffix + "'"
            # No prefix no suffix
            elif entry1[0] not in prefixSuffixList and entry1[-1] not in prefixSuffixList:
                name   = ' '.join(entry1[:-1])
                _type  = entry1[-1]
                               ##[NAME] = 'ROCKY' AND [TYPE] = 'ROAD'                    
                where_clause1 = "[NAME] = '" + name + "' AND [TYPE] = '" + _type + "'"
      
            entry2  = string.split(streetTwo)
            # Has prefix and suffix
            if entry2[0] in prefixSuffixList and entry2[-1] in prefixSuffixList:
                prefix = entry2[0]
                name   = ' '.join(entry2[1:-2])
                _type  = entry2[-2]
                suffix = entry2[-1]
                               ##[PREFIX] = '0' AND [NAME] = 'BAILEY' AND [TYPE] = 'CT' AND [SUFFIX] = '0'
                where_clause2 = "[PREFIX] = '" + prefix + "' AND [NAME] = '" + name + "' AND [TYPE] = '" + _type + "' AND [SUFFIX] = '" + suffix + "'"
            # Has prefix no suffix
            elif entry2[0] in prefixSuffixList and entry2[-1] not in prefixSuffixList:
                prefix = entry2[0]
                name   = ' '.join(entry2[1:-1])
                _type  = entry2[-1]
                               ##[PREFIX] = '0' AND [NAME] = 'ARMSTEAD' AND [TYPE] = 'LOOP'
                where_clause2 = "[PREFIX] = '" + prefix + "' AND [NAME] = '" + name + "' AND [TYPE] = '" + _type + "'"
            # No prefix has suffix
            elif entry2[0] not in prefixSuffixList and entry2[-1] in prefixSuffixList:
                name   = ' '.join(entry2[:-2])
                _type  = entry2[-2]
                suffix = entry2[-1]
                               ##[NAME] = 'ROGER PEED' AND [TYPE] = 'DR' AND [SUFFIX] = 'N'                    
                where_clause2 = "[NAME] = '" + name + "' AND [TYPE] = '" + _type + "' AND [SUFFIX] = '" + suffix + "'"
            # No prefix no suffix
            elif entry2[0] not in prefixSuffixList and entry2[-1] not in prefixSuffixList:
                name   = ' '.join(entry2[:-1])
                _type  = entry2[-1]
                               ##[NAME] = 'ROCKY' AND [TYPE] = 'ROAD'                    
                where_clause2 = "[NAME] = '" + name + "' AND [TYPE] = '" + _type + "'"

            global street1Entry
            global street2Entry
            street1Entry = street1.upper()
            street2Entry = street2.upper()
            ...
            try:
                ...
                # Apply search query
                arcpy.SelectLayerByAttribute_management(streetLayer, "NEW_SELECTION", where_clause1)
                # Get count of how many records met query
                ...
                if count != 0:
                    ...
                    arcpy.MakeFeatureLayer_management(Street, street1Selection, where_clause1)
                    clearSelectedFeatures()
                    try:
                        # Apply search query
                        arcpy.SelectLayerByAttribute_management(streetLayer, "NEW_SELECTION", where_clause2)
                        # Get count of how many records met query
                        ...
                        if count2 != 0:
                            ...
                            arcpy.MakeFeatureLayer_management(Street, street2Selection, where_clause2)
                            for lyr in arcpy.mapping.ListLayers(mxd,'',df):
...
class ToolClass_1_DataExtractor(object):
    ...
    def onRectangle(self, rectangle_geometry):
        Email('New Project Data Extractor Started')
        ...
        try:
            ...
            Email('New Project Data Extractor Finished')
        except:
            print 'Extraction Error'
            Email('New Project Data Extractor Error')
...
##Street Test
##        
##CHOICE_keyword1 = 'Jefferson Ave'
##CHOICE_keyword2 = 'George Washington Hwy'
##CHOICE_keyword3 = 'S 2nd St'
##CHOICE_keyword4 = '1st St N'
##CHOICE_keyword5 = 'E Jefferson ASDF Ave W'
##
##CHOICE_keyword6 = 'E I-64 ON Ramp'
##
##prefixSuffixList = ('N','S','E','W','0','U')
##
##x = CHOICE_keyword6
