...
import arcpy, pythonaddins, string, os, time, xlrd, xlwt
#import webbrowser
    ...
    def onRectangle(self, rectangle_geometry):
        ...
        if matchcount == 0:
            ...
            for drawing in selected_drawings:
                ...
                if fileType == 'DWG':
                    # Example File - 'S:\Docs\1990\00000240.DWG'
                    # Need to research how to add DWG file
                    print 'DWG file type detected: ' + a[-3] + '/' + a[-1].upper()
                    dwgCount += 1
                    totalCount += 1
                elif fileType == 'DXF':
                    # Example File - 'S:\Docs\2009\00000KU3.DXF'
                    # Need to research how to add DXF file
                    print 'DXF file type detected: ' + a[-3] + '/' + a[-1].upper()
                    dxfCount += 1
                    totalCount += 1
                elif fileType == 'TIF':
                    try:
                        print 'Added: ' + a[-3] + '/' + a[-1].upper()
                        arcpy.MakeRasterLayer_management(drawing, drawing[:-4])
                        for lyr in arcpy.mapping.ListLayers(mxd, drawing[:-4], df):
                            lyr.name = a[-3] + '/' + a[-1].upper()
                            lyr.visible = False
                            arcpy.RefreshTOC()
                            arcpy.RefreshActiveView()
                        tifCount += 1
            ...
            if tifCount > 0:
                groupLayer1 = arcpy.mapping.Layer(r'R:\Divisions\InfoTech\Shared\GIS\Python_Addins\addin_assistant\Projects\MyTools\NNWW_Toolbars\ProjectFiles\temp.lyr')
                arcpy.mapping.AddLayer(df, groupLayer1, "BOTTOM")
                groupLayer2  = arcpy.mapping.Layer(r'R:\Divisions\InfoTech\Shared\GIS\Python_Addins\addin_assistant\Projects\MyTools\NNWW_Toolbars\ProjectFiles\Drawings & Historical Orthophotographs.lyr')
                arcpy.mapping.AddLayer(df, groupLayer2, "BOTTOM")

                tempGroupLayer   = arcpy.mapping.ListLayers(mxd, 'temp', df)[0]
                targetGroupLayer = arcpy.mapping.ListLayers(mxd, 'Drawings & Historical Orthophotographs', df)[0]

                for lyr in arcpy.mapping.ListLayers(mxd, '*.TIF',df):
                    moveLayer = lyr
                    arcpy.mapping.AddLayerToGroup(df, tempGroupLayer, moveLayer, "BOTTOM")
                    arcpy.mapping.RemoveLayer(df, lyr)

                ABC_Order = []
                for lyr in tempGroupLayer:
                ...
                arcpy.mapping.RemoveLayer(df, tempGroupLayer)
                del ABC_Order
            ...
            if tifCountFail == 0 and dwgCount == 0 and dxfCount == 0:
                pythonaddins.MessageBox('\n\tTOTAL FILES PROCESSED\t\n\n\t\t' + str(totalCount) + '\n\n\n\n\n\tFILES ADDED\n\n\tTIF: \t' + str(tifCount), "NNWW Toolbars")
            elif tifCountFail == 0 and tifCount == 0: 
                pythonaddins.MessageBox('\n\tTOTAL FILES PROCESSED\t\n\n\t\t' + str(totalCount) + '\n\n\n\n\n\tFILES DETECTED\n\n\tDWG: \t' + str(dwgCount) + '\n\tDXF: \t' + str(dxfCount), "NNWW Toolbars")
            elif tifCountFail == 0:
                pythonaddins.MessageBox('\n\tTOTAL FILES PROCESSED\t\n\n\t\t' + str(totalCount) + '\n\n\n\n\n\tFILES ADDED\n\n\tTIF: \t' + str(tifCount) + '\n\n\tOTHER FILES DETECTED\n\n\tDWG: \t' + str(dwgCount) + '\n\tDXF: \t' + str(dxfCount), "NNWW Toolbars")
            else:
                pythonaddins.MessageBox('\n\tTOTAL FILES PROCESSED\t\n\n\t\t' + str(totalCount) + '\n\n\n\n\n\tFILES ADDED\n\n\tTIF: \t' + str(tifCount) + '\n\n\tOTHER FILES DETECTED\n\n\tDWG: \t' + str(dwgCount) + '\n\tDXF: \t' + str(dxfCount) + '\n\n\tERRORS\n\n\t' + str(tifCountFail) + ' TIF(s) failed to add.\n', "NNWW Toolbars")
